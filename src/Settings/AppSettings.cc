...
//2022729
const char* AppSettings::settingDirectory =         QT_TRANSLATE_NOOP("AppSettings", "Setting");
const char* AppSettings::userDirectory =            QT_TRANSLATE_NOOP("AppSettings", "User");
...
#endif
//修改不管如何都生成然后校验SavePath2022 7.29
...
#else
        QDir rootDir = QDir(QStandardPaths::writableLocation(QStandardPaths::DocumentsLocation));
        qDebug()<< rootDir.filePath(appName)<< "ROOTDIRFILENAME";
        savePathFact->setRawValue(rootDir.filePath(appName));
#endif
    }
...
void AppSettings::_checkSavePathDirectories(void)
{
    ...
    if (savePathDir.exists()) {
        ...
        savePathDir.mkdir(settingDirectory);
...
//2022729
QString AppSettings::settingSavePath(void)
{
    QString path = savePath()->rawValue().toString();
    if (!path.isEmpty() && QDir(path).exists()) {
        QDir dir(path);
        return dir.filePath(settingDirectory);
    }
    return QString();
}

QString AppSettings::userSavePath(void)
{
    QString path = savePath()->rawValue().toString();
    if (!path.isEmpty() && QDir(path).exists()) {
        QDir dir(path);
